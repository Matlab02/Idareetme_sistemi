...
(() => {
  ...
  function styles() {
    ...
    style.textContent = ".request-creation-documents{margin-top:2px}.request-creation-document-list{display:grid;grid-template-columns:repeat(3,1fr);gap:9px}.request-creation-document{display:grid;gap:7px;padding:11px;border:1px solid #dbe4f0;border-radius:10px;background:#fbfdff}.request-creation-document b{font-size:11px;color:#42536e}.request-creation-document input{position:absolute!important;width:1px!important;height:1px!important;opacity:0!important;pointer-events:none}.request-creation-upload{display:inline-flex;align-items:center;justify-content:center;min-height:32px;padding:0 10px;border:1px dashed #92b9e8;border-radius:8px;background:#fff;color:#1767ce;font-size:11px;font-weight:750;cursor:pointer;transition:.16s ease}.request-creation-upload:hover{border-color:#397ad2;background:#edf6ff}.request-creation-upload:focus-within{outline:3px solid #dbeaff}.request-creation-document .muted{margin:0;font-size:10px;overflow-wrap:anywhere}@media(max-width:760px){.request-creation-document-list{grid-template-columns:1fr}}";
  ...
  function addCreationDocuments() {
    ...
    section.innerHTML = `<label>Sorğu sənədləri <span class="muted">(istəyə bağlı)</span></label><div class="request-creation-document-list">${documentTypes.map(([type, label]) => `<div class="request-creation-document"><b>${label}</b><input id="new-request-doc-${type}" type="file" data-new-request-document="${type}" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png"><label class="request-creation-upload" for="new-request-doc-${type}">＋ Sənəd əlavə et</label><span class="muted">Hələ fayl seçilməyib</span></div>`).join("")}</div><small class="muted">Sorğu yaradıldıqdan sonra bu sənədləri istənilən vaxt yenidən yükləyə və dəyişə bilərsiniz.</small>`;
    form.append(section);
